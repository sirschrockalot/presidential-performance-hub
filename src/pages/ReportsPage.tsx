...
import { deals, draws, kpiEntries, getUserById, getUserPoints } from '@/data/mock-data';
...
const revenueByRep = ['u5', 'u6'].map(id => {
  ...
  return {
    name: user.name,
    revenue: fundedDeals.filter(d => d.acquisitionsRepId === id).reduce((s, d) => s + (d.assignmentFee || 0), 0),
...
const weeklyRevenue = [
  { week: '02-10', revenue: 20000, deals: 2 },
  { week: '02-17', revenue: 15000, deals: 1 },
  { week: '02-24', revenue: 32500, deals: 2 },
  { week: '03-03', revenue: 32500, deals: 2 },
  { week: '03-10', revenue: 25000, deals: 1 },
];
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard title="Draw Exposure" value={`$${draws.filter(d => ['paid', 'approved'].includes(d.status)).reduce((s, d) => s + d.remainingBalance, 0).toLocaleString()}`} icon={Banknote} variant="warning" />
      </div>
        ...
        <TabsContent value="revenue" className="mt-4 space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-2 rounded-lg border bg-card p-5">
              ...
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={dealsByStatus} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={55} paddingAngle={2} label={({ name, value }) => `${name}: ${value}`}>
              ...
              <tbody>
                {fundedDeals.map(d => (
                  <tr key={d.id} className="border-b last:border-b-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right hidden md:table-cell font-mono">{d.assignmentFee && d.contractPrice ? `${((d.assignmentFee / d.contractPrice) * 100).toFixed(1)}%` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="rounded-lg border bg-card p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {['u5', 'u6', 'u7', 'u8'].map(id => {
                ...
                const outstanding = repDraws.filter(d => ['paid', 'approved'].includes(d.status)).reduce((s, d) => s + d.remainingBalance, 0);
                return (
                  <div key={id} className="rounded-md border p-4">
                    ...
                    <p className="text-xs text-muted-foreground">{repDraws.length} total draws · {repDraws.filter(d => d.status === 'recouped').length} recouped</p>
                  </div>
                );
              })}
            </div>
                ...
                return (
                  <div key={id} className="rounded-md border p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold">{user.name}</p>
                      <p className="text-xs text-muted-foreground capitalize">{user.team} · {user.role.replace(/_/g, ' ')}</p>
                    </div>
