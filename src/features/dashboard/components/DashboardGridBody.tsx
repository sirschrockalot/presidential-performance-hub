...
import { DashboardGridLayout, type DashboardSlotKey } from "@/features/dashboard/components/DashboardGridLayout";
...
import type { PointsMetricsDto, PointsLeaderboardEntryDto } from "@/features/points/server/points.queries";
import type { DashboardRecentDealRow } from "@/features/deals/api/deals-client";

type DrawMetricsClient = {
  outstanding: number;
  pendingCount: number;
...
function formatWeekLabel(weekStarting: string | null | undefined) {
...
function hitTone(hitRatePercent: number) {
  return hitRatePercent >= 100
    ? "bg-success/10 text-success border-success/30"
    : hitRatePercent >= 90
      ? "bg-warning/10 text-warning border-warning/30"
      : "bg-destructive/10 text-destructive border-destructive/30";
...
function hitBadgeLabel(hitRatePercent: number) {
  return hitRatePercent >= 100 ? "Hit KPI" : hitRatePercent >= 90 ? "Near Miss" : "Missed KPI";
}

const DashboardECharts = dynamic(
  () => import("@/features/dashboard/components/DashboardECharts").then((m) => m.DashboardECharts),
  { ssr: false, loading: () => <DashboardChartsSkeleton /> }
);

const DashboardPipelineSlot = dynamic(
  () => import("@/features/dashboard/components/DashboardPipelineSlot").then((m) => m.DashboardPipelineSlot),
  {
    ssr: false,
    loading: () => <div className="min-h-[200px] rounded-lg border bg-muted/25 animate-pulse" aria-hidden />,
  }
);

const DashboardActivitySlot = dynamic(
  () => import("@/features/dashboard/components/DashboardActivitySlot").then((m) => m.DashboardActivitySlot),
  {
    ssr: false,
    loading: () => <div className="min-h-[200px] rounded-lg border bg-muted/25 animate-pulse" aria-hidden />,
  }
...
export type DashboardGridBodyProps = {
  metrics: DealMetricsDto;
  overview: DashboardOverviewDto;
  drawMetrics: DrawMetricsClient;
  pointsMetrics: PointsMetricsDto;
  roleCode: string | undefined;
  recentDeals: DashboardRecentDealRow[];
  topReps: PointsLeaderboardEntryDto[];
  revenueTrendDeltaPct: number;
};

function DashboardGridBodyComponent({
  metrics,
  overview,
  drawMetrics,
  pointsMetrics,
  ...
  recentDeals,
  topReps,
  revenueTrendDeltaPct,
}: DashboardGridBodyProps) {
  ...
  const slots = useMemo((): Record<DashboardSlotKey, React.ReactNode> => {
    return {
      metrics: (
        <div className="rounded-lg border bg-card p-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            />
            <MetricCard
              title="Avg Assignment Fee — Closed/Funded (MTD)"
              value={`$${Math.round(overview.avgAssignmentFeeThisMonth ?? 0).toLocaleString()}`}
              icon={TrendingUp}
              subtitle={`${overview.fundedDealsThisMonth ?? 0} funded · effective fee or spread`}
            />
            <MetricCard
              title="Outstanding Draws"
              value={`$${(drawMetrics.outstanding ?? 0).toLocaleString()}`}
              icon={Banknote}
              variant="warning"
            />
          </div>
      ...
      pipeline: <DashboardPipelineSlot overview={overview} />,
      charts: (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <DashboardECharts metrics={metrics} overview={overview} />
        </div>
      ),
      leaderboard: (
              ...
              <div className="text-xs text-muted-foreground mb-3">
                ...
                {kpiDashboard.topPerformer && (
                  <span
                    className={cn(
                      ...
                      complianceTone(kpiDashboard.topPerformer.compliancePercent)
                    )}
                  >
                ...
                {kpiDashboard.mostAtRisk && (
                  <span
                    className={cn(
                      "ml-2 inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold",
                      complianceTone(kpiDashboard.mostAtRisk.compliancePercent)
                    )}
                  >
                    {kpiDashboard.mostAtRisk.compliancePercent >= 50 ? "🟡" : "🔴"}{" "}
                    {kpiDashboard.mostAtRisk.compliancePercent.toFixed(0)}%
                  </span>
                )}
              </div>
                  ...
                  <div key={entry.repUserId} className="flex items-center gap-3">
                    ...
                    <span
                      className={cn(
                        ...
                        complianceTone(entry.compliancePercent)
                      )}
                    >
                      {entry.compliancePercent >= 100 ? "🟢" : entry.compliancePercent >= 50 ? "🟡" : "🔴"}{" "}
                      {entry.compliancePercent.toFixed(0)}%
                    </span>
                  </div>
                ))}
              </div>
      ...
      activity: <DashboardActivitySlot recentActivity={overview.recentActivity} />,
      insights: (
        <div className="space-y-4">
          ...
          <MetricCard
            ...
            icon={Star}
            subtitle={`Avg assignment fee (closed MTD): $${Math.round(overview.avgAssignmentFeeThisMonth ?? 0).toLocaleString()}`}
          />
          <MetricCard
                          ...
                          <div key={m.metricKey} className="flex items-center justify-between gap-3">
                            ...
                            {m.metricKey === "OFFERS_MADE" && kpiDashboard.offersTierBreakdown ? (
                              <div className="flex flex-wrap items-center gap-2 justify-end">
                                <span className="text-[11px] font-semibold font-mono text-muted-foreground">{m.hitLabel} hit</span>
              ...
              {kpiDashboard.offersTierBreakdown?.hitTarget.totalReps > 0 && (
                <div className="rounded-lg border bg-card p-4">
                  <div className="flex items-center justify-between gap-4">
                    <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Acq Offers (3–5/day)</h3>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    >
                      🔴 Below Minimum: {kpiDashboard.offersTierBreakdown.belowMinimum.reps}/
                      {kpiDashboard.offersTierBreakdown.belowMinimum.totalReps}
                    </span>
                    <span
                      className={cn(
                        "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold border",
                    ...
                    >
                      🟡 Met Minimum: {kpiDashboard.offersTierBreakdown.metMinimum.reps}/
                      {kpiDashboard.offersTierBreakdown.metMinimum.totalReps}
                    </span>
                    <span
                      className={cn(
                        "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold border",
                    ...
                    >
                      🟢 Hit Target: {kpiDashboard.offersTierBreakdown.hitTarget.reps}/
                      {kpiDashboard.offersTierBreakdown.hitTarget.totalReps}
                    </span>
                  </div>
                </div>
              )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-muted-foreground text-xs uppercase tracking-wider hidden md:table-cell">
                    Rep
                  </th>
  ...
  }, [metrics, overview, drawMetrics, pointsMetrics, pointsSubtitle, recentDeals, topReps, revenueTrendDeltaPct, kpiDashboard]);

  return <DashboardGridLayout slots={slots} />;
}

export const DashboardGridBody = memo(DashboardGridBodyComponent);
