...
import { Bar, BarChart, CartesianGrid, ResponsiveContainer, Tooltip as ReTooltip, XAxis, YAxis, Cell } from "recharts";
...
import { usePointEvents, usePointRecipients, useCreateManualPointAdjustment, useLeaderboard, usePointsMetrics } from "@/features/points/hooks/use-points";
...
export default function PointsLeaderboardPage() {
  ...
  // Note: we intentionally keep the leaderboard page focused; a full ledger lives at /points/history.
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="lg:col-span-1 rounded-lg border bg-card p-5">
        ...
        <div className="rounded-lg border bg-card p-5">
          <h3 className="text-sm font-semibold mb-4 flex items-center gap-2">
            <Trophy className="h-4 w-4 text-gold" /> Leaderboard (latest events)
          </h3>
          <DataTable columns={pointColumns} data={(events ?? []).slice(0, 10)} emptyMessage="No point events" />
        </div>
      </div>
    </div>
  );
