...
import { getCommissionEarned, getCommissionRate } from "@/features/commissions/types";
import type {
  CommissionWindow,
  PotentialRepSummary,
  RepWindowSummary,
...
export async function listRepWindowSummaries(
  ...
  for (const deal of fundedDeals) {
    const fee = deal.assignmentFee != null ? Number(deal.assignmentFee) : 0;
  ...
  const summaries: RepWindowSummary[] = Array.from(repMap.entries()).map(([repId, agg]) => ({
    ...
    commissionEarned: getCommissionEarned(agg.revenue),
    deals: agg.deals,
  }));
...
export async function listPotentialRepSummaries(
  ...
  for (const deal of assignedDeals) {
    const fee =
      deal.assignmentFee != null
        ? Number(deal.assignmentFee)
        : deal.assignmentPrice != null
          ? Number(deal.assignmentPrice) - Number(deal.contractPrice)
          : 0;
    const wd: WindowDeal = {
      dealId: deal.id,
      propertyAddress: deal.propertyAddress,
  ...
  const summaries: PotentialRepSummary[] = Array.from(repMap.entries()).map(([repId, agg]) => ({
    ...
    potentialCommission: getCommissionEarned(agg.revenue),
    deals: agg.deals,
  }));
